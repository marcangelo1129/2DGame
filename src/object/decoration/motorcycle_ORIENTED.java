 ...
 * @author Dangerouze
 */
public class motorcycle_ORIENTED extends SuperObject {
    Rectangle size;
    
    public motorcycle_ORIENTED(GamePanel gp, int worldX, int worldY, String orientation)
    {
        //object-specific variable assignment
        switch (orientation) {
            case "front":
                solidArea = new Rectangle(0,0,6,26);//Collision offset (x,y) and size (width,height) no need to edit if collision is false
                size = new Rectangle(16, 32);//image size
                name = "motorcycle_front";//image name
                break;
            case "back":
                solidArea = new Rectangle(0,0,6,26);//Collision offset (x,y) and size (width,height) no need to edit if collision is false
                size = new Rectangle(16, 32);//image size
                name = "motorcycle_back";//image name
                break;
            case "left":
                solidArea = new Rectangle(0,0,28,7);//Collision offset (x,y) and size (width,height) no need to edit if collision is false
                size = new Rectangle(32, 32);//image size
                name = "motorcycle_left";//image name
                break;
            case "right":
                solidArea = new Rectangle(0,0,28,7);//Collision offset (x,y) and size (width,height) no need to edit if collision is false
                size = new Rectangle(32, 32);//image size
                name = "motorcycle_right";//image name
                break;
        }
        collision = true;
