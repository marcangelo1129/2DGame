 ...
 * @author Dangerouze
 */
public class car_red_ORIENTED extends SuperObject {
    Rectangle size;
    
    public car_red_ORIENTED(GamePanel gp, int worldX, int worldY, String orientation)
    {
        ...
        switch (orientation) {
            case "front":
                ...
                name = "car_red_front";//image name
                break;
            case "back":
                solidArea = new Rectangle(0,0,18,32);//Collision offset (x,y) and size (width,height) no need to edit if collision is false
                size = new Rectangle(32, 32);//image size
                name = "car_red_back";//image name
                break;
            case "left":
                ...
                name = "car_red_left";//image name
                break;
            case "right":
                solidArea = new Rectangle(0,0,48,18);//Collision offset (x,y) and size (width,height) no need to edit if collision is false
                size = new Rectangle(48, 32);//image size
                name = "car_red_right";//image name
                break;
        }
        collision = true;
